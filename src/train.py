"""
Original implementation: https://github.com/pytorch/vision/blob/main/references/classification/train.py
"""
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, writer, model_ema=None, scaler=None):
    ...
    writer.add_scalar("img_per_sec/train", metric_logger.meters["img/s"].global_avg, epoch)


def evaluate(model, criterion, data_loader, device, writer, print_freq=100, log_suffix="", epoch=None):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    writer.add_scalar("Loss/val", metric_logger.meters["loss"].global_avg, epoch)
    writer.add_scalar("Acc1/val", metric_logger.meters["acc1"].global_avg, epoch)
    writer.add_scalar("Acc5/val", metric_logger.meters["acc5"].global_avg, epoch)
...
def main(args):
    ...
    if args.test_only:
        ...
        if model_ema:
            evaluate(model_ema, criterion, data_loader_test, device=device, writer=writer, log_suffix="EMA")
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, writer, model_ema, scaler)
        lr_scheduler.step()
        evaluate(model, criterion, data_loader_test, device=device, writer=writer, epoch=epoch)
        if model_ema:
            evaluate(model_ema, criterion, data_loader_test, device=device, writer=writer, log_suffix="EMA", epoch=epoch)
        if args.output_dir:
            checkpoint = {
